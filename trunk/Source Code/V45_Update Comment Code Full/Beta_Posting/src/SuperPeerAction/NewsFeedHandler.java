...
/**
 * NewsFeedHandler use to handle newsfeed (only for super peers)
 *
        ...
        if (respond_NewsFeed != null) {
            if (!respond_NewsFeedTable.containsKey(respond_NewsFeed.getMessageID())) {
                ...
                if (userIDInRes.equals(userIDLogin)) {
                    ...
                    // receive and show list of post in the newsfeed respond
                    if (listFeedRespond.trim().length() != 0) {
                        ...
                        // need sleep 3 seconds before show status
                        int count = 0;
                        long timeout = 1500;
                        while (true) {
                            try {
