...
/**
 * ProfileHandler uses to handle profile request and respond (only for super peers)
 * 
...
public class ProfileHandler extends Thread {
    ...
    public void run() {
        if (request_ProfileMsg != null) { 
            //check that request_ProfileMsg is not already in table
            if (!request_ProfileTable.containsKey(request_ProfileMsg.getMessageID())) 
            {
        ...
        if (respond_ProfileMsg != null) {
            if (!respond_ProfileTable.containsKey(respond_ProfileMsg.getMessageID())) //check that request_ProfileMsg is not already in table
            {
                ...
                String userIDInRes = respond_ProfileMsg.getUserIDPost();
                
                // if user logging in already requested the profile, it will receive
                if (userIDInRes.equals(AppGUI.idUserSelected)) {
                    String listPostRespond = respond_ProfileMsg.getListPost();
                    
                    // receive and show list of post in the profile respond
                    if (listPostRespond.trim().length() != 0) { 
                ...
                } else { // user logging in is not request the profile , then it will forward to other one
